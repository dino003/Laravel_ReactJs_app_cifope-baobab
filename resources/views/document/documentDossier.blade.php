...
<?php endif ?>
      ...
      <?php if ($role->ajout==1): ?>
     <a href="#">
      <em class="" onclick="$('#ajouterfichier').modal('show');"><span>Ajouter un document pour le service</span></em>
     </a>  
      <?php endif ?>
  ...
  <td><a href="{{url('deleteDocumentServie')}}/{{$param1}}" onclick="if(! confirm('Voulez vous supprimer le document?'))return false;" title="Supprimer"><font color="#000"><i class="fa fa-trash"></i></font></a></td>
   ...
   <?php endif ?>
	...
	<div class="col-md-4">
		...
		@include('document.listeSousdossier')
	</div>
...
<!--Creation de sous dossier--->
 ...
 <div class="modal-dialog">
       <div class="modal-content">
        ...
        <form method="post" action="" enctype="multipart/form-data" id="formulaire589">
   {{ csrf_field() }}                         
			...
			<input type="hidden" name="structure_id" value="{{$structure->id}}">
                          ...
                          <div class="modal-footer">
                                            <button type="button" class="btn btn-default" data-dismiss="modal">Fermer</button>     
                                               <button type="submit" class="btn btn-info has-spinner1000000 " id="ajouter">Ajouter</button>
                                            </div>
       </form>
                  </div>
      </div>
...
<form method="post" action="" id="formulaire1452" enctype="multipart/form-data" >
          {{ csrf_field() }}
...
<input type="file" accept=".jpeg,.jpg,.png,.gif,.doc,.dot,.docx,.dotx,.docm,.xlsx,.xlsm,.xlsb,.xltx,.xltm,.xls,.xlt,.xml,.xlam,.xla,.xlw,.xlr,.pptx,.pptm,.ppsm,.ppt,.potx,.potm,.pot,.ppsx,.pps,.ppam,.ppa,.mde,.accde,.adp,.accdp,.accdr,.one,.onepkg,.vsd,.vst,.vsdm,.mpt,.mpp,.pub,.pdf,.rar,.zip" name="file[]" id="image5" multiple>
...
<div class="modal-footer">
  <button type="button" class="btn btn-default" data-dismiss="modal">Fermer</button>     
                                                <button type="submit" class="btn btn-primary has-spinner2" name="modifier" id="ajoute5">Ajouter</button>
                    </div>
 </form>
<!--Fin Ajout--->
...
$(document).ready(function () {
      $("#ajoute5").prop('disabled', true);
  $( "#image5" ).change(function() {
   $("#ajoute5").prop('disabled', false);
});
...
$(document).ready(function () {
    ...
    $('.has-spinner1000000').click(function (e) {
        var btn = $(this);
        $(btn).buttonLoader('start');
        e.preventDefault();
        var formData = new FormData($(this).parents('form')[0]);
